...
module WikiFormatting
  module Macros
    ...
    macro :child_pages do |obj, args|
      ...
      page = nil
      if args.size > 0
        page = Wiki.find_page(args.first.to_s, :group => @group)
      elsif obj.is_a?(WikiContent)
        page = obj.page
      else
        raise 'With no argument, this macro can be called from wiki pages only.'
      end
      raise 'Page not found' if page.nil? || !User.current.allowed_to?(:view_wiki_pages, page.wiki.group)
      pages = ([page] + page.descendants).group_by(&:parent_id)
      render_page_hierarchy(pages, options[:parent] ? page.parent_id : page.id)
    end
    
    desc "Include a wiki page. Example:\n\n  !{{include(Foo)}}\n\nor to include a page of a specific group wiki:\n\n  !{{include(groupname:Foo)}}"
