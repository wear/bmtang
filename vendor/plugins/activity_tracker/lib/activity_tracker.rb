module ActivityTracker # :nodoc:
  ...
  module ActMethods
      ...
      unless included_modules.include? InstanceMethods
        after_create do |record|
          unless options[:if].kind_of?(Proc) and not options[:if].call(record)
            group = (options[:group].nil? ?  '' : options[:group].call(record)) 
            record.create_activity_from_self(group) 
          end
        end
    ...
    # 
    # Extended so that when type = group givend it will trac group
    #
    # Examples: 
  ...
  module InstanceMethods
    ...
    def create_activity_from_self(group)
